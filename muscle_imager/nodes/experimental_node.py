...
from trigger_node import trigger_node
...
class ExperimentalNode(object):
	...
	def __init__(self):
		...
		# ae additionopto_stimulus
		#self.closed_loop_gain = 19.0 #19.0 for stripe #4? for camo
		#self.closed_loop_gain = -19.0 #after ivo switched orientation of kinefly
		#self.closed_loop_gain = -18.0
		self.closed_loop_gain = 17.0
		rospy.logwarn('closed loop gain ' + str(self.closed_loop_gain))
		self.closed_loop_bias = 0.0
		self.open_loop_gain = 20.0
		...
		# Trigger node
		self.trigger_pub = trigger_node()

		#self.hfDur = .0625 #ivo
		#self.hfDur = .125 #ivo
		self.hfDur = .25 #ivo
		rospy.logwarn('ivo_mode only: Chrimson pulse duration set to ' + str(self.hfDur*2) + ' s')
	...
	def fire_trigger_pulse(self):
		...
		#self.ctrl.set_ao(3,value = 0.0)
		#rospy.sleep(0.0001)
		self.trigger_pub.publish_hs_trigger()
	...
	def opto_stimulus(self):
		...
		#self.ctrl.set_ao(4,value=0.0)
		#rospy.sleep(0.0001)
		self.trigger_pub.publish_opto_trigger_high()
		rospy.sleep(0.25)
		self.trigger_pub.publish_hs_trigger()
		rospy.sleep(0.25)
		self.trigger_pub.publish_opto_trigger_low()

	# # 5s for measuring laser power
	# def opto_stimulus(self):
	# 	self.ctrl.set_ao(4, value=5.0)
	# 	rospy.sleep(2.5)
	# 	self.ctrl.set_ao(3, value=5.0)
	...
	def opto_stimulus_ivo(self):
		self.ctrl.set_ao(4,value=5.0)
		rospy.sleep(self.hfDur)
		#rospy.sleep(0.125)
		self.ctrl.set_ao(3,value=5.0)
		rospy.sleep(0.0001)
		self.ctrl.set_ao(3,value=0.0)
	...
	def LED_cmd_callback(self,msg):
		if msg.data == 'loom_only_mode':
			...
			self.opto_stimulus()
		elif msg.data == 'opto_pulse_ivo': #ivo
			self.opto_stimulus_ivo()
