...
class frame_counter(object):
    ...
    def __init__(self):
        ...
        # Publish a phase message
        self.pubPhaseMsg = rospy.Publisher(self.nodename+'/phase_msg', UInt32, queue_size=1000)

        # Publish a wb duration message
        self.pubWbMeanMsg = rospy.Publisher(self.nodename+'/wb_T_msg', UInt32, queue_size=1000)
    ...
    def new_frame_nr_callback(self,msg):
        ...
        if msg_mode == 'N':
            ...
            self.pubPhaseMsg.publish(np.uint32(msg_count))
        elif msg_mode == 'W':
            self.pubWbMeanMsg.publish(np.uint32(msg_count))
